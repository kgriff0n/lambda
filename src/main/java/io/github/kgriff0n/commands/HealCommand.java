...
public class HealCommand {
    public static void register() {
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) -> {
            dispatcher.register(literal("heal")
                    .requires(Permissions.require("lambda.admin.heal", PermissionLevel.GAMEMASTERS))
    ...
    private static int execute(CommandSourceStack source, ServerPlayer target) {
        ...
        if (player == target) {
            player.sendSystemMessage(Component.nullToEmpty(Config.healSelf));
        } else {
            player.sendSystemMessage(Component.nullToEmpty(String.format(Config.healOthers, target.getName())));
            target.sendSystemMessage(Component.nullToEmpty(Config.healSelf));
        }
